...
void Demo::Render() {
   ...
   // ----------------------------------------
	...
	lightView = glm::lookAt(glm::vec3(-2.0f, 4.0f, -1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0, 1.0, 0.0)); // -2.0f, 4.0f, -1.0f
...
void Demo::BuildTexturedCube() {
	...
	int width, height;
	unsigned char* image = SOIL_load_image("kulkass.png", &width, &height, 0, SOIL_LOAD_RGBA);
...
void Demo::BuildTexturedPlane()
{
	...
	int width, height;
	unsigned char* image = SOIL_load_image("planks.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
