...
void Demo::BuildTexturedCube() {
	...
	int width, height;
	unsigned char* image = SOIL_load_image("kulkas_diffusemap0.png", &width, &height, 0, SOIL_LOAD_RGBA);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("kulkas_specularmap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		...
		// front
		-0.707, -0.5, 0, 0, 0,  0.0f,  0.0f,  1.0f, // 0 kiri bawah
		0, -0.5, 0.707, 0.5, 0,   0.0f,  0.0f,  1.0f, // 1 kanan bawah
		0,  1.5, 0.707, 0.5, 1, 0.0f,  0.0f,  1.0f, // 2 kanan atas
		-0.707,  1.5, 0, 0, 1, 0.0f,  0.0f,  1.0f, // 3 kiri atas
		// x    y    z  | x   y

		// right
		0,  -0.5,  0.707, 0.5, 0,  1.0f,  0.0f,  0.0f, // 4
		0.707,  -0.5, 0, 1, 0,   1.0f,  0.0f,  0.0f, // 5
		0.707, 1.5, 0, 1, 1,   1.0f,  0.0f,  0.0f, // 6
		0, 1.5,  0.707, 0.5, 1,   1.0f,  0.0f,  0.0f, // 7

		// back
		0.707,  -0.5, 0, 0.5, 0, 0.0f,  0.0f,  -1.0f, // 8 
		0,  -0.5, -0.707, 1, 0,0.0f,  0.0f,  -1.0f,  // 9
		0,   1.5, -0.707, 1, 1, 0.0f,  0.0f,  -1.0f, // 10
		0.707, 1.5, 0, 0.5, 1, 0.0f,  0.0f,  -1.0f, // 11

		// left
		0, -0.5, -0.707, 0.5, 0, -1.0f,  0.0f,  0.0f, // 12
		-0.707, -0.5,  0, 1, 0, -1.0f,  0.0f,  0.0f, // 13
		-0.707,  1.5,  0, 1, 1, -1.0f,  0.0f,  0.0f, // 14
		0,  1.5, -0.707, 0.5, 1, -1.0f,  0.0f,  0.0f, // 15

		// upper
		-0.707, 1.5,  0, 0.5, 0,  0.0f,  1.0f,  0.0f,  // 16
		0, 1.5,  0.5, 1, 0, 0.0f,  1.0f,  0.0f,  // 17
		0.707, 1.5, 0, 1, 0.5, 0.0f,  1.0f,  0.0f,  // 18
		0, 1.5, 0.707, 0.5, 0.5,  0.0f,  1.0f,  0.0f,  // 19

		// bottom
		-0.707, -0.5, 0, 0.5, 0, 0.0f,  -1.0f,  0.0f, // 20
		0, -0.5, 0.707, 1, 0,   0.0f,  -1.0f,  0.0f, // 21
		0.707, -0.5,  0, 1, 0.5,   0.0f,  -1.0f,  0.0f, // 22
		0, -0.5,  0.707, 0.5, 0.5,  0.0f,  -1.0f,  0.0f, // 23
	};
...
void Demo::DrawTexturedCube()
{
	...
	glm::mat4 model;
	model = glm::translate(model, glm::vec3(0, 0, 0)); //translasi(x, y,z)

	model = glm::rotate(model, angle, glm::vec3(0, 1, 0)); //rotasi(x, y, z)

	model = glm::scale(model, glm::vec3(1, 1, 1)); //ukuran(x, y, z)
...
void Demo::BuildTexturedPlane()
{
	...
	int width, height;
	unsigned char* image = SOIL_load_image("planks_diffusemap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("planks_specularmap.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
...
int main(int argc, char** argv) {
	...
	app.Start("Tugas 3", 800, 600, false, false);
}